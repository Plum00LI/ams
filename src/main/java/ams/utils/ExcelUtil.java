...
public class ExcelUtil {
    ...
    public static void exportExcel(Map<String, Object> paramMap, HttpServletResponse response, IBaseDao dao, String[] headList,
                                   String[] describeList) {
        SXSSFWorkbook workbook = new SXSSFWorkbook(100);

        // 获取总个数
        Integer totalCount = dao.likeSelectCount(paramMap).intValue();
        ...
        for (int i = 0; i < sheetCount; i++) {
            ...
            for (; m < totalPage; m++) {
                ...
                // 查询数据库的数据
                // sql中的start
                paramMap.put("start", m * 5000);
                // 5000条
                paramMap.put("pageSize", 5000);

                List<Map<String, Object>> resultList = dao.likeSelect(paramMap);

                // 设置内容
                for (int resultIndex = 0; resultIndex < resultList.size(); resultIndex++) {

                    Map<String, Object> resultMap = resultList.get(resultIndex);
                    ...
                    for (int headIndex = 0; headIndex < headList.length; headIndex++) {
                        ...
                        if (resultMap == null) {
                            ...
                            Object[] array = resultMap.values().toArray();
                            currentValue = String.valueOf(array[headIndex]);
                            // 获取当前字段的描述信息
                            describe = describeList[headIndex];
                        }
